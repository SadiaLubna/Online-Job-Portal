...
<?php 
   ...
   $related_jobs = $conn->query("SELECT * FROM jobs  job_category WHERE= '$row->job_category' AND status = 1 AND id != '$id'");
   ...
   $application_deadline = strtotime($row->application_deadline); // Assuming deadline is May 15, 2023
   $current_date = time(); // Current date and time
   $is_application_deadline_passed = ($application_deadline < $current_date);
        ...
        <div class="row">
          <div class="col-lg-8">
            ...
            <?php if($checking_for_application->rowCount() == 0) : ?>
              ...
              <div class="col-6">
              <?php if ($is_application_deadline_passed): ?>
    <button style="padding: 13px 157px; margin-top: -17px;" type="button" class="btn btn-inline btn-secondary btn-md" id="apply-btn">Apply</button>
    <script>
        function showApplicationClosedAlert() {
            alert("Application is closed");
        }

        document.querySelector('#apply-btn').addEventListener('click', function(event) {
            event.preventDefault();
            showApplicationClosedAlert();
        });
    </script>
<?php else: ?>
          ...
          <div class="col-lg-4">
            <div class="bg-light p-3 border rounded mb-4">
              ...
              <ul class="list-unstyled pl-3 mb-0">
                <li class="mb-2"><strong class="text-black">Published on: </strong><?php echo date('M', strtotime($row->created_at))  . ',' . date('d', strtotime($row->created_at)) . ' ' . date('Y', strtotime($row->created_at)); ?></span></li>
                <li class="mb-2"><strong class="text-black">Vacancy: </strong><?php echo $row->vacancy; ?></span></li>
            ...
            <div class="bg-light p-3 border rounded mb-4 mt-4">
              ...
              <ul class="list-unstyled pl-3 mb-0">
               ...
               <a target="_blank" style="text-decoration: none" href="<?php echo APPURL; ?>/categories/show-jobs.php?name=<?php echo ucfirst($category->name); ?>"><li class="mb-2"><strong class="text-black"><?php echo $category->name; ?></li></a>
               <?php endforeach; ?>
