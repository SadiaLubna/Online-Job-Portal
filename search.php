...
if(isset($_POST['submit'])) {
    ...
    if(empty($_POST['job-title']) OR empty($_POST['job-region']) OR empty($_POST['job-type'])) {
      echo "<script>alert('one input or more are empty')</script>";
        ...
        $search = $conn->query("SELECT * FROM jobs WHERE job_title LIKE '%$job_title%' AND job_region 
        LIKE '%$job_region%' AND job_type LIKE '%$job_type%' AND status = 1");
        ...
        <?php if(count($searchRes) > 0) : ?>
          ...
          <li class="job-listing d-block d-sm-flex pb-3 pb-sm-0 align-items-center">
            ...
            <div class="job-listing-logo">
              <img src="users/user-images/<?php echo $oneJob->company_image; ?>" alt="<?php echo $oneJob->company_image; ?>" class="img-fluid">
            </div>
          ...
          <?php endforeach; ?>
          <?php else : ?>
            <div class="alert alert-danger bg-danger text-white">There is no jobs with this searches yet, try to search with other inputs</div>
            <?php endif ; ?>
